...
> **Start here (this machine)**: read `docs/DECISIONS.md` first — it holds the decision log, verified benchmark numbers, and local-environment pitfalls. Keep this file lean; put new detail there.
...
- `app/routers/` — API routes: `note.py` (generation), `provider.py`, `model.py`, `config.py`, `chat.py` (RAG Q&A on generated notes), `sync.py` (multi-device note sync keyed by a plain user id)
...
- `store/` — Zustand stores: `taskStore`, `syncStore` (optional user-id based sync), `modelStore`, `configStore`, `providerStore`. Persists to IndexedDB.
...
Full detail in `docs/DECISIONS.md`. Essentials:

- **Services**: WSL2 systemd units `bilinote-backend` (:8483) and `bilinote-frontend` (:3015); currently `active` but `disabled` (no boot autostart). Restart: `sudo systemctl restart bilinote-backend`.
- **Python**: backend runs on the in-repo venv `backend/myvenv` (3.12); system-wide environments are off-limits. `myvenv/bin/pip` has a broken shebang → use `myvenv/bin/python -m pip`.
- **ASR**: GPU-accelerated (`RTX 5070 Ti` + `faster-whisper` `large-v3-turbo`). Verify with `curl http://127.0.0.1:8483/api/deploy_status`; falls back to CPU automatically when the NVIDIA libs fail to load.
- **Line endings**: `app/routers/config.py`, `app/services/note.py`, `app/transcriber/transcriber_provider.py` are CRLF; normalize to LF before using `apply_patch`.
- **Frontend**: Tailwind **v4**; the root `tailwind.config.cjs` is a dead v3 leftover with no effect. Mobile layout uses JS branches via `src/hooks/useIsMobile.ts`, not CSS hiding.
- **Hard constraint**: never modify system-wide environment; confirm with the user before high-risk operations.
